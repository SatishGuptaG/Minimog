...
var defaultCountry = "";
var defaultSymbol = "";
Vue.component("section-pricing", {
    template: "#section-pricing-template",
    data() {
        return {
            ...
            isPricingEditDisabled: !hasPricingEditPermission
        };
    },
    props: ['message'],
    async beforeCreate() {
...
methods: {
    ...
    checkValidPrice: function () {
        var self = this;
        var valid = true;
        if (Number(self.formModel.sellPrice) < Number(self.formModel.sellPriceExcVat)) {
            valid = false;
            self.formModel.sellPrice = "";
            Vue.toasted.show('Sell Price should be greater then sell price excluding tax', { type: 'error', duration: 3000, dismissible: true });
        }
        if (valid == false) {
            this.$emit('validation-complete', false);
        }
