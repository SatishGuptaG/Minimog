...
const axiosInstanceVariantAttribute = function () {
    const getBaseUrl = () => {
        return PimUri;
    };
...
Vue.component("section-variantattributes", {
    template: "#section-variantattributes-template",
    props: ['message'],
    data() {
        return {
            //customFields: [],
    ...
    methods: {
        ...
        confirmUpdateAttributeSet() {
            ...
            self.$modal.show('dialog', {
                title: 'Confirmation !',
                height: '100',
                text: 'Do you really want to Update Attribute set?',
                buttons: [
                    {
                        title: 'Ok',
        ...
        loadSurvey: function (id) {
            ...
            directAxios()
                .get("/MasterData/VariantAtributeData?id=" + id)
                .then(function (response) {
                    ...
                    if (response.data &&
                        response.data.variantFormAttribute.pages &&
                        response.data.variantFormAttribute.pages.length &&
                        response.data.variantFormAttribute.pages[0] &&
                        response.data.variantFormAttribute.pages[0].elements &&
                        response.data.variantFormAttribute.pages[0].elements.length) {

                        self.formModel.survey = response.data.variantFormAttribute.pages[0].elements;

                        EventBus.$emit('attributeData.loaded', response.data);
                    } else {
                        self.formModel.survey = [];
                        document.getElementById("variantcustomFields").style.display = "none";
                        document.getElementById("variantcustomFieldsLI").style.display = "none";
                    }
                        ...
                        for (var i = 0; i < self.formModel.customFieldData.length; i++) {
                            ...
                            for (var j = 0; j < self.formModel.survey.length; j++) {
                                ...
                                if (currentSurvey.name === currentValue.key) {
                                        ...
                                        for (var k = 0; k < currentSurvey.choices.length; k++) {
                                            ...
                                            var selectedColors = currentValue.value == null ? [] : currentValue.value.split(',');
                                                for (var x = 0; x < selectedColors.length; x++) {
                                                    if (col1.value != null && col1.value.toLowerCase() === selectedColors[x].toLowerCase()) {
                                                        col1.selected = true;
                                    ...
                                    if (currentSurvey.type === "checkbox") {
                                    ...
                                    if (currentSurvey.type === "dropdown") {
                                        if (currentValue.value != null) {
