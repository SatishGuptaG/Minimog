...
Vue.component("section-advpurchasedatareadonly", {
    template: "#section-advpurchasedatareadonly-template",
    data() {
        return {
            formModel: {
                advPurchCostExFac: 0,
    ...
    mounted() {
        ...
        EventBus.$on("product.loaded", function (product) {
            ...
            self.productpurchasedatamaster = product.advancedPurchaseData;
            ...
            if (advPurchase != null) {
                ...
                self.taxClassName = (advPurchase != null && advPurchase.taxClass != null) ? advPurchase.taxClass.value : '';
                self.formModel.selectedCountry = self.preSelectedCountry;
                setTimeout(function () {
            ...
            //setTimeout(function () {
            //    if (self.taxClassListLoaded == true) {
            //        for (var i = 0; i < self.taxClassList.length; i++) {
            //            var currentTC = self.taxClassList[i];
            //            if (currentTC.value == self.formModel.taxClass) {
            //                self.taxClassName = currentTC.name;
            //            }
            //        }
            //    }
            //}, 4000);
    ...
    methods: {
        ...
        advPurchaseDataUpdateModel() {
            var self = this;
            self.$modal.show('advPurchaseDataUpdateModel');
        },
        hide() {
            var self = this;
            self.setdata();
            self.$modal.hide('advPurchaseDataUpdateModel');
        },
        beforeClose() {
            var self = this;
            if (!successed) {
        ...
        setdata() {
            var self = this;  
            self.editProductData = true; 
            var advPurchase = self.productpurchasedatamaster;
      
            self.preSelectedCountry = advPurchase != null ? advPurchase.countryCode : self.formModel.selectedCountry;
            self.preSelectedCurrSymbol = advPurchase != null ? advPurchase.currencySymbol : self.formModel.selectedCurrency;
            self.preSelectedCurr = advPurchase != null ? advPurchase.currencyCode : self.formModel.selectedCurrSymbol;
            self.getSuppliersByCountry(advPurchase.countryCode);
            ...
            if (advPurchase != null) {
                ...
                self.formModel.advPurchReorderQty = advPurchase.reorderQty;
                self.formModel.advPurchLeadTime = advPurchase.leadTime;
                self.formModel.selectedCountry = advPurchase.countryCode;
                self.preSelectedCountry = (advPurchase.countryCode != null) ? advPurchase.countryCode : "";
                self.formModel.selectedCurrency = advPurchase.currencyCode;
                self.preSelectedCurr = (advPurchase.currencyCode != null) ? advPurchase.currencyCode : "";
                self.formModel.selectedCurrSymbol = advPurchase.currencySymbol;
        ...
        calculateLandingCost: function () {
            var self = this;
            if (self.formModel.advPurchDutyCharges == null || self.formModel.advPurchDutyCharges == 0) {
                self.calculateDutyCharges();
            }
        ...
        setexchangerateReadonly: function () {
            var self = this;
            calculateCostPrice();
            setTimeout(function () {
                self.formModel.advPurchInsurance = advInsurance.toFixed(2);
                        ...
                        if (selectedSupplier != null && selectedSupplier != "") {
                            setTimeout(function () {
                                ...
                                for (var i = 0; i < self.suppList.length; i++) {
                                    if (self.suppList[i].recordId == selectedSupplier) {
                                        self.supplierName = self.suppList[i].name;
                                    }
                                }
                            }, 500);
                        }
        ...
        getCountryList: async function () {
            ...
            if (countries != null && countries.length > 0) {
                for (var i = 0; i < countries.length; i++) {
                    if (countries[i].symbol != null) {
                        ...
                        if (countries[i].name == 'United Kingdom') {
                            ...
                            defaultSymbol = countries[i].symbol;
                            selectedCountry = countries[i].code;
                            self.countryName = countries[i].name;
                        }
                    }
                }
                self.countriesLoaded = true;
                ...
                self.formModel.selectedCurrency = self.preSelectedCurr;
                for (var i = 0; i < countries.length; i++) {
                    if (self.formModel.selectedCountry == countries[i].code) {
                        self.countryName = countries[i].name;
                    }
                }
                self.getSuppliersByCountry(self.formModel.selectedCountry);
            ...
            setTimeout(function () {
                ...
                var productModel = {
                    ...
                    advancedPurchaseData: {
                        isAdvancedPurchaseFeatureEnabled: true,
                        costPriceExFactory: self.formModel.advPurchCostExFac,
                        exchangeRate: self.formModel.advPurchExRate,
                        costPrice: self.formModel.advPurchCostPrice,
                        freight: self.formModel.advPurchFreight,
                        dutyableCost: self.formModel.advPurchDutyableCost,
                        dutyCharges: self.formModel.advPurchDutyCharges,
                        insurance: self.formModel.advPurchInsurance,
                        costQC1: self.formModel.advPurchCostQC1,
                        costQC2: self.formModel.advPurchCostQC2,
                        cost1: self.formModel.advPurchCost1,
                        cost2: self.formModel.advPurchCost2,
                        defectsProvision: self.formModel.advPurchDefectsProvision,
                        LandingCostIncludingQC: self.formModel.advPurchLandingCostIncludingQC,
                        rrp: self.formModel.advPurchRRP,
                        minimumOrderQuantity: self.formModel.advPurchMinimumOrderQuantity,
                        reorderLevel: self.formModel.advPurchReorderLevel,
                        reorderQty: self.formModel.advPurchReorderQty,
                        leadTime: self.formModel.advPurchLeadTime,
                        countryCode: self.formModel.selectedCountry,
                        currencyCode: self.formModel.selectedCurrency,
                        currencySymbol: self.formModel.selectedCurrSymbol,
                        landingCostFormula: self.formModel.advPurchLandingCostFormula,
                        supplierId: self.formModel.selectedSupplier,
                        sellPrice: self.formModel.sellPrice,
                        taxClass: {
                            key: self.formModel.taxClass
                        }
                    }
                };

                axiosInstancePost()
                    .post('ProductEditor/UpdateProductAdvPurchasedata', productModel, axiosConfig)
                    .then(function (response) {
                        ...
                        if (data.isValid) {
                            ...
                            for (var i = 0; i < self.countiresList.length; i++) {
                                if (self.formModel.selectedCountry == self.countiresList[i].id) {
                                    self.countryName = self.countiresList[i].text;
                                }
                            }
                            for (var i = 0; i < self.suppList.length; i++) {
                                if (self.suppList[i].RecordId == self.formModel.selectedSupplier) {
                                    self.supplierName = self.suppList[i].name;
                                }
                            }
                        }
